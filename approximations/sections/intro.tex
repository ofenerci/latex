...
%%%
\newcommand{\lpos}{x} % linear position
...
\subsubsection{Rules of thumb on nondim}
\begin{enumerate}
...
\subsubsection{DA and similarity}
\begin{enumerate}
%
...
nondim parameters can be generated by several methods. One of them is the \lingo{method of repeating variables}. It consists in six steps:
%
\begin{enumerate}
%
...
%
\item set the reduction $j$ as the \lingo{number of primary dims}. and calculate $k$, the expected number of $\kdim$'s ($k = n 
...
\subsubsection{Guidelines for choosing repeating parameters}
\begin{itemize}
%
...
\begin{itemize}
%
\item Variables having only $\phdim L$ in their dimension are called \lingo{geometric variables}.
%
\item Variables having only $\phdim T$ or both $\phdim T$ and $\phdim L$ are called \lingo{kinematic variables}.
%
\item Variables having $\phdim M$ in their dimension are called \lingo{dynamic variables}.
%
\end{itemize}
