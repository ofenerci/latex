...
# == Description
# wrapper around PHP pChart library v.2.1.3
#
# == Author
# rimbaud1854
#
...
class PChartPlotter < Object
  ...
  php_plot = <<-EOS
    <?php   
     ...
     /* Write the title */
     //$myPicture->setFontProperties(array("FontName"=>"./pChart2.1.3/fonts/Forgotte.ttf","FontSize"=>23));
  ...
  File.open './plot.php', 'w' do |file| 
    file.write php_plot
  end
  
  `php -q ./plot.php`
  #{}`rm ./plot.php`
  #{}`/usr/local/bin/convert ./pictures/example.basic.png ./pictures/example.basic.pdf`
  
end

plotter = PChartPlotter.new
plotter
