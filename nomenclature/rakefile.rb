...
EXTLESS_SOURCE = File.basename(SRC, '.tex')
...
task :tex_to_pdf do 
  ...
  sh "makeindex #{EXTLESS_SOURCE}.nlo -s nomencl.ist -o #{EXTLESS_SOURCE}.nls"
...
'*.acn', '*.glo', '*.ilg', '*.ist', '*.lof', '*.lot', '*.ntn', '*.toc', 
'*.idx', '*.nlo', '*.nls', '*.out', '*.acr', '*.bbl', '*.blg', '*.brf', 
'*.gls', '*.not', '*.nav', '*.snm', '*.gz', '*.synctex.gz*')
