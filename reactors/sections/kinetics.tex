...
\lingo{Reaction kinetics} is the branch of chemistry that quantifies reaction rates. But, even though this branch treats all types of chemical reactions, we limit herein the discourse to \lingo{elementary reactions} -- reactions whose rates correspond to stoichiometric equations.
...
\beq
-\rrate = \krcoeff\prod_{i = 1}^{k}\conc{R,$i$}^{\kstnum_i}\,,
\eeq
where $\krcoeff$ is the \lingo{reaction rate coefficient}, $\conc{R,$i$}$ the $i$-th reactant concentration and the sign is due to the fact that reactants are \emph{consumed}. If we were, on the other hand, to express the reaction rate as a function of the products:
\beq
+\rrate = \krcoeff\prod_{j = 1}^{l}\conc{P,$j$}^{\kstnum_j}\,.
\eeq

By adding the $\elset{\kstnum_i}$, define the overall \lingo{reaction order}, $\rorder$:
\beq
