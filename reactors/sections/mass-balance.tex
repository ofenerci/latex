...
The last statement represents the key point in \lingo{mass transfer}: analogously to the force balance in statics, the mass balance allows mass flow quantification and verification in chemical systems.
...
Given the reactor volume $\vol$ and the initial concentration of a chemical species $\ce A$, $\conc{A,0}$, inside the reactor, the total mass of $\ce A$ will be $\mass_{\ce A} = \vol\conc{A,0}$. In the unit time, $\conc A$ will change only by virtue of a chemical reaction. The mass balance quantifies such a change:
\beq
...
whose solution is
\beq
\conc A/\conc{A,0} = \exp\vat{-\krcoeff t}\,,
\eeq
...
\beq
\conc A/\conc{A,0} = 1 /\left(1 + \krcoeff\conc{A,0}t\right)\,.
\eeq
...
A CSTR differs from a batch only in the fact that the former is not closed. Thus, the terms in the mass balance representing the mass of a species $\ce A$, $\mass_{\ce A}$, flowing in and flowing out will not cancel:
\beq
...
\eeq
where $\rtime = \vol/\flow\vol$ is the average \lingo{hydraulic residence time}. The last equation represents the \lingo{characteristic equation for a CSTR}. 

Finally, assuming a \lingo{first-order reaction}, the model becomes
\beq
\conc{A,out}/\conc{A,in} = 1/\left(1 + \krcoeff\rtime\right)\implies
...
% ------------------------------------------------------------- Figure
% position: bthH. size:width=0.5\textwidth. file:location+filename
 \docfigure{bt}{width=0.7\textwidth}{./figures/pf-reactor.pdf}%
   {PFR}
