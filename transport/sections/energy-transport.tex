...
Consider a body of mass $m$ and volume $\vol$ being heated by an inflow of \lingo{thermal energy}, \lingo{thermal flow} or \lingo{thermal power}, $\flow\then$. Due to heating, the body expands and performs \lingo{work} onto the surroundings at a rate $\rate\work$. While both processes take place, on the other hand, the body \lingo{internal energy} $\ien$ changes, or accumulates, at a rate $\rate\ien$ given by the \lingo{energy conservation principle}:
\beq
...
\ien = \kshcap\mass\temp\,,
\eeq
where $\kshcap$ is a property of the body material called \lingo{specific heat capacity}~\footnote{~This property should really be called \lingo{specific energy capacity}, for ``heat'' is no more than energy in transit!}, or the capacity that a body has to store or release thermal energy, $\dim\kshcap = E/M\Theta$, and $\temp$ is the body thermodynamic temperature.
...
\flow\then = 
    ...
    \end{cases}
\eeq
where $\kshcap_\vol$ is the \lingo{specific heat capacity at constant volume} and $\kshcap_\press$ the \lingo{specific heat capacity at constant pressure}.
